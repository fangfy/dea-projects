...
def s1_orbit(filename):
    """
    Derive absolute orbit number from filename
    """
    sensor = filename.split('_')[0]
    relative = int(filename.split('_')[6])
    if sensor =='S1A': return 'S1A', (relative - 73)%175 + 1
    if sensor =='S1B': return 'S1B', (relative  - 27)%175 + 1
