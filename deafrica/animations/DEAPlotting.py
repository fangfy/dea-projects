...
def animated_timeseries(ds, output_path, 
                        ...
                        title=False, show_date=True, annotation_kwargs={}, animation_options = {},
    ...
    if time_dim in ds and x_dim in ds and y_dim in ds:        
        ...
        if ((len(bands) == 3) | (len(bands) == 1)) & all([(b in ds.data_vars) for b in bands]): 
            ...
            ani = animation.FuncAnimation(fig, update_figure, frames=timesteps, interval=interval, blit=True, **animation_options)
    ...
    if time_dim1 in ds1 and x_dim1 in ds1 and y_dim1 in ds1 and time_dim2 in ds2 and x_dim2 in ds2 and y_dim2 in ds2:
        ...
        if ((len(bands1) == 3) | (len(bands1) == 1)) & all([(b1 in ds1.data_vars) for b1 in bands1]) & \
           ((len(bands2) == 3) | (len(bands2) == 1)) & all([(b2 in ds2.data_vars) for b2 in bands2]):  
            ...
            if output_path[-3:] == 'mp4':
                print('    Exporting animation to {}'.format(output_path))
                ani.save(output_path, dpi=width_pixels / width)
                ...
                print('    Exporting animation to {}'.format(output_path))
                ani.save(output_path, dpi=width_pixels / width, writer='imagemagick')
...
def _ds_to_arrraylist(ds, bands, time_dim, x_dim, y_dim, percentile_stretch, image_proc_func=None, vmin=None, vmax=None): 
    ...
    if vmin is None and vmax is None:
        ...
        #p_lows, p_highs = np.nanpercentile(ds[bands].to_array(), np.array(percentile_stretch)*100, axis=np.arange(len(ds.dims))+1)
